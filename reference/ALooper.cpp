...
status_t ALooper::stop() {
    ...
    {//非常机智的做法，先把需要的变量cache下来，然后脱锁等待线程退出。把锁控制到最小范围
        Mutex::Autolock autoLock(mLock);
    ...
    mQueueChangedCondition.signal();//为啥这里不lock？
    {
...
void ALooper::post(const sp<AMessage> &msg, int64_t delayUs) {
    ...
    if (it == mEventQueue.begin()) {//只在队列为空的时候才发signal，估计是节省耗时吧
...
status_t ALooper::postReply(const sp<AReplyToken> &replyToken, const sp<AMessage> &reply) {
    ...
    if (err == OK) {
        mRepliesCondition.broadcast();//mRepliesCondition不区分reply token，也就是唤醒所有在等待的reply token
    }
    return err;
}
